...
function testGrader(score, possible) {
    if ((score / possible) >= .90) { return ("A") }
    else if ((score / possible) >= .80) { return ("B") }
    else if ((score / possible) >= .70) { return ("C") }
    else if ((score / possible) >= .60) { return ("D") }
    else { return ("F") }
}
// --------------------------------------------
...
function timeOfDayGreeting(hour) {
    ...
    else if ((hour >= 22) && (hour <= 24) || (hour >= 1) && (hour <= 4)) { return ("good night") }
...
function isFever(temp) {
    let health = "fever"
    let good = "no "
    let hot = " go to hospital"
    if (temp >= 103) { return (health + hot) }
    else if (temp > 98.6) { return health }
    else { return (good + health) }
